...
if __name__=='__main__':
    ...
    agent_bQ = np.load('生存率数据/Q值-ICU的-45/agent_bQ.npy')
    phys_bQ = np.load('生存率数据/Q值-ICU的-45/phys_bQ.npy')
    shencunlv1 = np.load('生存率数据/Q值-ICU的-45/shencunlv.npy')
    ...
    while i <= 25:
        ...
        # print(count)
        # print('11')
        # count = phys_df.loc[(agent_bQ[0] > i - 0.5) & (agent_bQ[0] < i + 0.5)]
        try:
    ...
    plt.plot(bin_medians, sliding_mean(mort), color='b')
    plt.fill_between(bin_medians, sliding_mean(mort) - 1 * mort_std,
                     sliding_mean(mort) + 1 * mort_std, color='lightblue')
    ...
    plt.title('The reward with 45 observation features',fontname="Arial")
